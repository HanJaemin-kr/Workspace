...
class GA:
    ...
    def solve_ttp_problem(self, mutation_rate=0.01):
        ...
        if(True):
            ...
            print(" > Selected Knapsack Weights:",
                  sum([self.item_weights[i] for i in range(len(best_individual['kp_genome'])) if
                       best_individual['kp_genome'][i] == 1]))

            print(" > Total Item Value:", sum([self.item_values[i] for i in range(len(best_individual['kp_genome'])) if
                                               best_individual['kp_genome'][i] == 1]))
            total_distance = 0
